...
# Function generating DGP-8
dgp_8 <- function(n, cluster_size, discard = 20) {
  ...
  for (i in 1:cluster_size[1]) {
    ...
    ts <- arima.sim(n = total_length, model = list(ar = c(0.8), ma = c(-0.4)), innov = rnorm(total_length))
    data_matrix[, i] <- ts[(discard + 1):total_length]
  }
  
  for (i in (cluster_size[1]+1):(total_no_series-cluster_size[3])) {
    
    ts <- arima.sim(n = total_length, model = list(ma = c(0.5)), innov = rnorm(total_length))
    data_matrix[, i] <- ts[(discard + 1):total_length]
  }
  
  for (i in ((total_no_series-cluster_size[3])+1):total_no_series) {
    
    ts <- arima.sim(n = total_length, model = list(ar = c(1.4, -0.45)), innov = rnorm(total_length))
    data_matrix[, i] <- ts[(discard + 1):total_length]
  }
  return(data_matrix)
}
...
for (iteration in 1:num_iter) {
  ...
  data_matrix <- dgp_8(sample_size, cluster_size_sim8)
...
print("Empirical Probability for Simulation-8 Jump Statistic:")
